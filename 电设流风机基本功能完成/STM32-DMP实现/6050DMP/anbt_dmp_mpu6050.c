...
u8 AnBT_DMP_MPU6050_Init(void)	
{
	...
	//
	if(!mpu_set_sensors(INV_XYZ_GYRO | INV_XYZ_ACCEL)) AnBT_Uart1_Send_String("2.DMP设置传感器成功",19);		//完成传感去设定
	else AnBT_Uart1_Send_String("2.DMP设置传感器错误",19);
	//
	if(!mpu_configure_fifo(INV_XYZ_GYRO | INV_XYZ_ACCEL)) AnBT_Uart1_Send_String("3.设置FIFO成功",14);		//完成fifo设置
	else AnBT_Uart1_Send_String("3.设置FIFO失败",14);
	//
	if(!mpu_set_sample_rate(DEFAULT_MPU_HZ))AnBT_Uart1_Send_String("4.DMP设置采样率成功",19);			//完成DMP采样率设置
	else AnBT_Uart1_Send_String("4.DMP设置采样率失败",19);
	//
	if(!dmp_load_motion_driver_firmware())	AnBT_Uart1_Send_String("5.DMP加载运动驱动固件成功",25);				//开启DMP功能
	else AnBT_Uart1_Send_String("5.DMP加载运动驱动固件失败",25);
	//
	if(!dmp_set_orientation(inv_orientation_matrix_to_scalar(gyro_orientation))) AnBT_Uart1_Send_String("6.DMP完成设置初始方向设定偏差",29);		//初始方向设定偏差
	else AnBT_Uart1_Send_String("6.DMP设置初始方向设定偏差出错",29);
	//
	if(!dmp_enable_feature(DMP_FEATURE_6X_LP_QUAT | DMP_FEATURE_TAP |
	DMP_FEATURE_ANDROID_ORIENT | DMP_FEATURE_SEND_RAW_ACCEL | DMP_FEATURE_SEND_CAL_GYRO |
	DMP_FEATURE_GYRO_CAL))		AnBT_Uart1_Send_String("7.DMP使能成功",13);		//DMP使能
	else AnBT_Uart1_Send_String("7.DMP使能出错",13);
	//
	if(!dmp_set_fifo_rate(DEFAULT_MPU_HZ)) AnBT_Uart1_Send_String("8.DMP设置FIFO速率成功",21);		//DMP  fifo速度
	else AnBT_Uart1_Send_String("8.DMP设置FIFO速率失败",21);
	//
	run_self_test();
	if(!mpu_set_dmp_state(1)) AnBT_Uart1_Send_String("9.Dmp启动成功",13);				//DMP成功开启
